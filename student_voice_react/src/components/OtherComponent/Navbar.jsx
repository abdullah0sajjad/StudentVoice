...
function Navbar() {
   ...
   const {isLoading, data} = useQuery('UserNotification', async ()=>{
  ...
  useEffect(()=>{
    if (Object.keys(notificationUsers).length !== 0) {
      const unreadNotifications = notificationUsers.notifications.filter((notification) => !notification.isRead);
      ...
      <div className="container-fluid">
        ...
        <div className={isOpen ? "collapse navbar-collapse show" : "collapse navbar-collapse"} id="navbarNav">
          <ul className="navbar-nav nav-icons">
            ...
            <li className="nav-item">
              <Link className="nav-link" to={"/notifications"} onClick= {(e)=> setIsOpen(false)}><i className='p-overlay-badge'><IoIosNotifications /> {user ? <Badge value={notificationCount} severity="danger"></Badge> : ''}</i> </Link>
            </li>
            <li className="nav-item">
