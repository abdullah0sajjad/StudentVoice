...
module.exports.google = async (req, res) => {
    ...
    try {
        const user = await User.findOne({email})
        if (user) {
          const uu =  await User.updateOne({email: email}, {$set: {currentSemester: current_semester, imageUrl: imageUrl}})
          console.log(uu, current_semester)  
          const token = createToken(user);
            return res.status(200).json({msg: 'Login Successfully', token: token});
        }else{
            try {
...
module.exports.notificationUser = async (req, res) => {
...
module.exports.markAsReadNotification = async (req, res) => {
    const { userId } = req.params;
    const {notifyId} = req.body;
    
    try {
        const user = await User.findById(userId);
        const selectedNotification = user.notifications.find((notify)=> notify._id.equals(notifyId));
...
module.exports.removeNotification = async (req, res) => {
    ...
    try {
        const user = await User.findById(userId);
        const selectedNotification = user.notifications.find((notify) => notify._id.toString() === notifyId);

        if (selectedNotification) {
          user.notifications = user.notifications.filter((notify) => notify._id.toString() !== notifyId);
          await user.save();
        } else {
            return res.status(404).json({ msg: 'Notification not found' });
        }
...
module.exports.markAsAllReadNotification = async (req, res) => {
    const { userId } = req.params;
...
module.exports.removeAllNotifications = async (req, res) => {
    const { userId } = req.params;
    ...
    try {
        ...
        return res.status(200).json({ msg: 'All notifications removed successfully!' });
    } catch (error) {
        console.log(error);
        return res.status(500).json({errors: error, msg: error.message});
    }
